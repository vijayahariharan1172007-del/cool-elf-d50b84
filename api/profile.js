...
module.exports=async(req,res)=>{try{
 ...
 const [direct,teamById,teamBySnake]=await Promise.all([
  ...
  supabase.from('event_registrations').select('id,event,event_key,status,created_at,master_id,team_members').contains('team_members',[{masterId:id}]).order('created_at',{ascending:false}),
  supabase.from('event_registrations').select('id,event,event_key,status,created_at,master_id,team_members').contains('team_members',[{master_id:id}]).order('created_at',{ascending:false})
 ]);
 if(direct.error)throw direct.error;
 if(teamById.error)throw teamById.error;
 if(teamBySnake.error)throw teamBySnake.error;
 const byId=new Map();
 for(const row of [...(direct.data||[]),...(teamById.data||[]),...(teamBySnake.data||[])])byId.set(String(row.id),row);
